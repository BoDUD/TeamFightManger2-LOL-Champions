...
- Run: eight stable compact cross-step frames preserve the native frame count and 0.065 s timing while following the Viktor reference principle. The accepted upper model, horns, shoulders, and sword remain unchanged; only the lower foot pixels alternate left/right so the animation reads as a cross-step instead of a zombie shuffle.
