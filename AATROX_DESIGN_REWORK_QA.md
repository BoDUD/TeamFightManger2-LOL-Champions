...
- Compact and battle surfaces: Aatrox core actor actions are repacked into Viktor-like 57x54 frames, but the visible body is kept smaller than Viktor because horns, shoulders, and sword make his silhouette heavier. Idle/run stay around 34 px tall, attack/skill around 35 px, with at least 16 px of bottom transparent safety so the feet sit above the health/name label instead of under it.
- Run: eight stable compact cross-step frames preserve the native frame count and 0.065 s timing while following the Viktor reference contract: one base `run` action is authored, and the game mirrors that action for the opposite facing direction. The accepted upper model, horns, shoulders, and lowered sword remain unchanged; the lower-body pixels are taken from existing accepted Aatrox action frames so the walk has leg-over-leg motion without code-drawn thin legs or a new oversized model.
