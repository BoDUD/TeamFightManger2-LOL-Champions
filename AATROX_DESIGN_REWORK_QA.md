...
- Run: eight stable compact cross-step frames preserve the native frame count and 0.065 s timing while following the Viktor reference contract: one base `run` action is authored, and the game mirrors that action for the opposite facing direction. The approved model, horns, shoulders, and greatsword remain from the same source sheet; no code-drawn legs, line swords, or oversized replacement bodies are allowed.
