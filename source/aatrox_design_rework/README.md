...
This folder documents the accepted image-gen source route for the design-book Aatrox rebuild.
...
These source PNGs are not committed after the rebuild is complete. Keep them only as local scratch inputs when regenerating the runtime assets, then export the final runtime sheets directly into `aseprite_resources/`, `icons/`, and `text/`.
...
Temporary contact and preview sheets are local-only diagnostics and must be deleted before publishing.
